...
static void recv_bounce_msg(void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO(zmq_getsockopt(socket_, ZMQ_RCVMORE, &rcvmore, &sz));
...
{
    int timeout = 250;
    TEST_ASSERT_SUCCESS_ERRNO(zmq_setsockopt(socket_, ZMQ_SNDTIMEO, &timeout, sizeof(int)));
...
static void recv_bounce_msg_fail(void *socket_)
{
    int timeout = 250;
    char buffer[32];
    TEST_ASSERT_SUCCESS_ERRNO(zmq_setsockopt(socket_, ZMQ_RCVTIMEO, &timeout, sizeof(int)));
...
void s_send_seq(void *socket_, ...)
{
    ...
    while (true)
    {
        ...
        if (!prev)
        {
            TEST_ASSERT_SUCCESS_ERRNO(zmq_send(socket_, 0, 0, end ? 0 : ZMQ_SNDMORE));
        }
        else
        {
            TEST_ASSERT_SUCCESS_ERRNO(zmq_send(socket_, prev, strlen(prev) + 1, end ? 0 : ZMQ_SNDMORE));
        }
        if (end)
            break;
    }
...
void s_recv_seq(void *socket_, ...)
{
    ...
    while (true)
    {
        ...
        TEST_ASSERT_SUCCESS_ERRNO(zmq_getsockopt(socket_, ZMQ_RCVMORE, &more, &more_size));
...
fd_t connect_socket(const char *endpoint_, const int af_, const int protocol_)
{
    ...
    socklen_t addr_len;
    const fd_t s_pre = socket(af_, SOCK_STREAM, protocol_ == IPPROTO_UDP ? IPPROTO_UDP : protocol_ == IPPROTO_TCP ? IPPROTO_TCP : 0);
...
fd_t bind_socket_resolve_port(const char *address_, const char *port_, char *my_endpoint_, const int af_, const int protocol_)
{
    ...
    if (af_ == AF_INET || af_ == AF_INET6)
    {
        ...
        TEST_ASSERT_SUCCESS_RAW_ERRNO(setsockopt(s_pre, SOL_SOCKET, SO_REUSEADDR, &flag, sizeof(int)));
    ...
    if (af_ == AF_INET || af_ == AF_INET6)
    {
        ...
        sprintf(my_endpoint_, "%s://%s:%u",
            ...
            : protocol_ == IPPROTO_WSS ? "wss"
                                       : "ws",
            address_, af_ == AF_INET ? ntohs((*(struct sockaddr_in *)&addr).sin_port) : ntohs((*(struct sockaddr_in6 *)&addr).sin6_port));
    }
