...
int test_assert_success_message_errno_helper(int rc_, const char *msg_, const char *expr_, int line_)
{
    if (rc_ == -1)
    {
        ...
        snprintf(buffer, sizeof(buffer) - 1, "%s failed%s%s%s, errno = %i (%s)", expr_, msg_ ? " (additional info: " : "", msg_ ? msg_ : "",
...
int test_assert_success_message_raw_errno_helper(int rc_, const char *msg_, const char *expr_, int line_, bool zero)
{
    if (rc_ == -1 || (zero && rc_ != 0))
    {
        ...
        char buffer[512];
        buffer[sizeof(buffer) - 1] = 0; // to ensure defined behavior with VC++ <= 2013
        snprintf(buffer, sizeof(buffer) - 1, "%s failed%s%s%s with %d, errno = %i/%s", expr_, msg_ ? " (additional info: " : "", msg_ ? msg_ : "",
...
int test_assert_failure_message_raw_errno_helper(int rc_, int expected_errno_, const char *msg_, const char *expr_, int line_)
{
    ...
    if (rc_ != -1)
    {
        snprintf(buffer, sizeof(buffer) - 1,
            ...
            expr_, msg_ ? " (additional info: " : "", msg_ ? msg_ : "", msg_ ? ")" : "", expected_errno_, rc_);
        UNITY_TEST_FAIL(line_, buffer);
    }
    else
    {
        ...
        if (current_errno != expected_errno_)
        {
            snprintf(buffer, sizeof(buffer) - 1,
                ...
                expr_, msg_ ? " (additional info: " : "", msg_ ? msg_ : "", msg_ ? ")" : "", expected_errno_, current_errno);
            UNITY_TEST_FAIL(line_, buffer);
        }
    }
    return rc_;
...
void recv_string_expect_success(void *socket_, const char *str_, int flags_)
{
    ...
    const int rc = TEST_ASSERT_SUCCESS_ERRNO(zmq_recv(socket_, buffer, sizeof(buffer), flags_));
    TEST_ASSERT_EQUAL_INT((int)len, rc);
    if (str_)
...
void test_bind(void *socket_, const char *bind_address_, char *my_endpoint_, size_t len_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO(zmq_getsockopt(socket_, ZMQ_LAST_ENDPOINT, my_endpoint_, &len_));
}
...
void bind_loopback(void *socket_, int ipv6_, char *my_endpoint_, size_t len_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO(zmq_setsockopt(socket_, ZMQ_IPV6, &ipv6_, sizeof(int)));
