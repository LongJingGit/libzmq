...
static bool is_multicast_available(int ipv6_)
{
    ...
    if (ipv6_)
    {
        ...
        rc = setsockopt(bind_sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, as_setsockopt_opt_t(&mreq), sizeof(mreq));
        if (rc < 0)
        {
            goto out;
        }
        ...
        int loop = 1;
        rc = setsockopt(send_sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, as_setsockopt_opt_t(&loop), sizeof(loop));
        if (rc < 0)
        {
            goto out;
        }
    ...
    else
    {
        ...
        rc = setsockopt(bind_sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, as_setsockopt_opt_t(&mreq), sizeof(mreq));
        if (rc < 0)
        {
            goto out;
        }
        ...
        int loop = 1;
        rc = setsockopt(send_sock, IPPROTO_IP, IP_MULTICAST_LOOP, as_setsockopt_opt_t(&loop), sizeof(loop));
        if (rc < 0)
        {
            goto out;
        }
    ...
    rc = sendto(send_sock, msg, static_cast<socklen_t>(strlen(msg)), 0, &mcast.generic, sl);
    if (rc < 0)
    {
        goto out;
    }
...
{
#ifdef _WIN32
    TEST_IGNORE_MESSAGE("ZMQ_MULTICAST_LOOP=false does not appear to work on Windows (TODO)");
#endif
    ignore_if_unavailable(ipv6_);
    ...
    int loop = 0;
    TEST_ASSERT_SUCCESS_ERRNO(zmq_setsockopt(radio, ZMQ_MULTICAST_LOOP, &loop, sizeof(int)));
