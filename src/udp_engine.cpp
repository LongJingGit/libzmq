    ...
    if (_send_enabled)
    {
        if (!_options.raw_socket)
        {
            ...
            if (out->is_multicast())
            {
                ...
                rc = rc | set_udp_multicast_loop(_fd, is_ipv6, _options.multicast_loop);

                if (_options.multicast_hops > 0)
                {
                    rc = rc | set_udp_multicast_ttl(_fd, is_ipv6, _options.multicast_hops);
                }
...
#ifdef ZMQ_HAVE_VXWORKS
        rc = rc | bind(_fd, (sockaddr *)real_bind_addr->as_sockaddr(), real_bind_addr->sockaddr_len());
#else
        rc = rc | bind(_fd, real_bind_addr->as_sockaddr(), real_bind_addr->sockaddr_len());
#endif
        if (rc != 0)
        {
            assert_success_or_recoverable(_fd, rc);
...
int zmq::udp_engine_t::set_udp_multicast_ttl(fd_t s_, bool is_ipv6_, int hops_)
{
    ...
    const int rc = setsockopt(s_, level, IP_MULTICAST_TTL, reinterpret_cast<char *>(&hops_), sizeof(hops_));
    assert_success_or_recoverable(s_, rc);
    return rc;
}
...
int zmq::udp_engine_t::set_udp_multicast_iface(fd_t s_, bool is_ipv6_, const udp_address_t *addr_)
{
    ...
    if (is_ipv6_)
    {
        ...
        if (bind_if > 0)
        {
            ...
            rc = setsockopt(s_, IPPROTO_IPV6, IPV6_MULTICAST_IF, reinterpret_cast<char *>(&bind_if), sizeof(bind_if));
        }
    }
    else
    {
        ...
        {
            rc = setsockopt(s_, IPPROTO_IP, IP_MULTICAST_IF, reinterpret_cast<char *>(&bind_addr), sizeof(bind_addr));
        }
    }
...
{
    int on = on_ ? 1 : 0;
    const int rc = setsockopt(s_, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char *>(&on), sizeof(on));
    assert_success_or_recoverable(s_, rc);
    return rc;
}
...
#else
    int on = on_ ? 1 : 0;
    int rc = setsockopt(s_, SOL_SOCKET, SO_REUSEPORT, reinterpret_cast<char *>(&on), sizeof(on));
    assert_success_or_recoverable(s_, rc);
    return rc;
#endif
}
...
int zmq::udp_engine_t::add_membership(fd_t s_, const udp_address_t *addr_)
{
    ...
    if (mcast_addr->family() == AF_INET)
    {
        ...
        rc = setsockopt(s_, IPPROTO_IP, IP_ADD_MEMBERSHIP, reinterpret_cast<char *>(&mreq), sizeof(mreq));
    }
    else if (mcast_addr->family() == AF_INET6)
    {
        ...
        rc = setsockopt(s_, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, reinterpret_cast<char *>(&mreq), sizeof(mreq));
    }
...
void zmq::udp_engine_t::in_event()
{
    ...
    const int nbytes = recvfrom(_fd, _in_buffer, MAX_UDP_MSG, 0, reinterpret_cast<sockaddr *>(&in_address), &in_addrlen);
