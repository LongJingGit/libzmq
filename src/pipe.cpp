...
int zmq::pipepair(object_t *parents_[2], pipe_t *pipes_[2], const int hwms_[2], const bool conflate_[2])
{
    ...
    /**
     * attention：这里创建了 pipes_[0] 和 pipes_[1]。并且分别设置了读写的无锁队列 upipe1 upipe2
     *
     * pipes_[0] : upipe1 作为 input，upipe2 作为 output
     * pipes_[1] : upipe1 作为 output，upipe2 作为 input
     *
     * 经过这种设置，A 模块通过 pipes_[0].upipe1 写入, B 模块通过 pipes_[1].upipe1 读出
     *
     */
...
void zmq::pipe_t::process_pipe_peer_stats(uint64_t queue_count_, own_t *socket_base_, endpoint_uri_pair_t *endpoint_pair_)
{
...
void zmq::pipe_t::set_disconnect_msg(const std::vector<unsigned char> &disconnect_)
{
    _disconnect_msg.close();
