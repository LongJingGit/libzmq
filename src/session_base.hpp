...
public:
    ...
    static session_base_t *create(
        zmq::io_thread_t *io_thread_, bool active_, zmq::socket_base_t *socket_, const options_t &options_, address_t *addr_);
...
private:
    ...
    // 实例化 session 的时候初始化的. 对于 TCP 协议来说, 一般 server 初始化为 false, client 初始化为 true; 对于 UDP 协议来说，都初始化为 true
    const bool _active;

    //  Pipe connecting the session to its socket.
    zmq::pipe_t *_pipe; // IO 线程和 socket 进行数据交换的管道（session 和 socket 通信的 pipe）
