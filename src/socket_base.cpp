...
zmq::socket_base_t::socket_base_t(ctx_t *parent_, uint32_t tid_, int sid_, bool thread_safe_)
    ...
    , _monitor_sync()
{
    options.socket_id = sid_; // 每个 socket 拥有自己唯一的 id
...
#    endif
        && options.type != ZMQ_PUB && options.type != ZMQ_SUB && options.type != ZMQ_XPUB && options.type != ZMQ_XSUB)
    {
    ...
    if (protocol_ == protocol_name::udp && (options.type != ZMQ_DISH && options.type != ZMQ_RADIO && options.type != ZMQ_DGRAM))
    {
        errno = ENOCOMPATPROTO;
        return -1;
    }
...
#    endif
    ...
    if (protocol == protocol_name::udp)
    {
        ...
        add_endpoint(endpoint_uri_pair_t(endpoint_uri_, std::string(), endpoint_type_none), static_cast<own_t *>(session), newpipe);
...
#    ifdef ZMQ_HAVE_WSS
    ...
    {
        ws_listener_t *listener = new (std::nothrow) ws_listener_t(io_thread, this, options, protocol == protocol_name::wss);
#    else
    if (protocol == protocol_name::ws)
    {
        ws_listener_t *listener = new (std::nothrow) ws_listener_t(io_thread, this, options, false);
#    endif
        alloc_assert(listener);
        rc = listener->set_local_address(address.c_str());
        if (rc != 0)
...
int zmq::socket_base_t::connect_internal(const char *endpoint_uri_)
{
    ...
    if (protocol == protocol_name::inproc)
    {
        ...
        const int sndhwm = peer.socket == NULL                               ? options.sndhwm
                           : options.sndhwm != 0 && peer.options.rcvhwm != 0 ? options.sndhwm + peer.options.rcvhwm
                                                                             : 0;
        const int rcvhwm = peer.socket == NULL                               ? options.rcvhwm
                           : options.rcvhwm != 0 && peer.options.sndhwm != 0 ? options.rcvhwm + peer.options.sndhwm
                                                                             : 0;
        ...
        // ::pipepair 中会创建 pipe_t 和输入输出的无锁队列，并设置 socket 的读写端
        rc = pipepair(parents, new_pipes, hwms, conflates);
        if (!conflate)
        {
    ...
    if (protocol == protocol_name::tcp)
    {
        ...
        {
            check++;
            while (isalnum(*check) || isxdigit(*check) || *check == '.' || *check == '-' || *check == ':' || *check == '%' || *check == ';' ||
                   *check == '[' || *check == ']' || *check == '_' || *check == '*')
            {
                check++;
            }
        }
...
#    endif
    ...
    else if (protocol == protocol_name::tipc)
    {
        ...
        const sockaddr_tipc *const saddr = reinterpret_cast<const sockaddr_tipc *>(paddr->resolved.tipc_addr->addr());
    ...
    const bool subscribe_to_all =
        protocol == protocol_name::pgm || protocol == protocol_name::epgm || protocol == protocol_name::norm || protocol == protocol_name::udp;
#elif defined ZMQ_HAVE_OPENPGM
    const bool subscribe_to_all = protocol == protocol_name::pgm || protocol == protocol_name::epgm || protocol == protocol_name::udp;
#elif defined ZMQ_HAVE_NORM
    const bool subscribe_to_all = protocol == protocol_name::norm || protocol == protocol_name::udp;
#else
    ...
    // 发送命令将 session 绑定到 IO 线程，IO 线程会将 session 含有的句柄加入到 Poller 中，由 IO 线程侦听读写事件
    // 注意：系统级客户端 connect 是在 zmq::session_base_t::start_connecting 中完成的
...
int zmq::socket_base_t::recv(msg_t *msg_, int flags_)
{
    ...
    /**
     * 主线程在 while 循环中等待命令。
     * IO 线程从 kernel 接收完消息，会将消息写入到 pipe 中，然后会发送命令给主线程。
     * 主线程接收到 IO 线程的命令，从 pipe 中读取消息
     */
    while (true)
    {
...
void zmq::socket_base_t::event(const endpoint_uri_pair_t &endpoint_uri_pair_, uint64_t values_[], uint64_t values_count_, uint64_t type_)
{
    scoped_lock_t lock(_monitor_sync);
    // _monitor_events 在 zmq::socket_base_t::monitor 接口中被赋值（zmq::socket_base_t::monitor 由应用程序进行调用）
...
void zmq::socket_base_t::monitor_event(
    uint64_t event_, const uint64_t values_[], uint64_t values_count_, const endpoint_uri_pair_t &endpoint_uri_pair_) const
{
...
const zmq::routing_socket_base_t::out_pipe_t *zmq::routing_socket_base_t::lookup_out_pipe(const blob_t &routing_id_) const
{
    // TODO we could probably avoid constructor a temporary blob_t to call this function
