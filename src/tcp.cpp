...
int zmq::tune_tcp_keepalives(fd_t s_, int keepalive_, int keepalive_cnt_, int keepalive_idle_, int keepalive_intvl_)
{
    ...
    if (keepalive_ != -1)
    {
        ...
        const int rc = WSAIoctl(s_, SIO_KEEPALIVE_VALS, &keepalive_opts, sizeof(keepalive_opts), NULL, 0, &num_bytes_returned, NULL, NULL);
        assert_success_or_recoverable(s_, rc);
        if (rc == SOCKET_ERROR)
            return rc;
    }
...
int zmq::tune_tcp_maxrt(fd_t sockfd_, int timeout_)
{
    ...
    const int rc = setsockopt(sockfd_, IPPROTO_TCP, TCP_MAXRT, reinterpret_cast<char *>(&timeout_), sizeof(timeout_));
    assert_success_or_recoverable(sockfd_, rc);
    return rc;
...
int zmq::tcp_write(fd_t s_, const void *data_, size_t size_)
{
    ...
    if (nbytes == SOCKET_ERROR && (last_error == WSAENETDOWN || last_error == WSAENETRESET || last_error == WSAEHOSTUNREACH ||
...
#    if !defined(TARGET_OS_IPHONE) || !TARGET_OS_IPHONE
        errno_assert(errno != EACCES && errno != EBADF && errno != EDESTADDRREQ && errno != EFAULT && errno != EISCONN && errno != EMSGSIZE &&
                     errno != ENOMEM && errno != ENOTSOCK && errno != EOPNOTSUPP);
#    else
        errno_assert(errno != EACCES && errno != EDESTADDRREQ && errno != EFAULT && errno != EISCONN && errno != EMSGSIZE && errno != ENOMEM &&
...
int zmq::tcp_read(fd_t s_, void *data_, size_t size_)
{
    ...
    if (rc == SOCKET_ERROR)
    {
        ...
        {
            wsa_assert(last_error == WSAENETDOWN || last_error == WSAENETRESET || last_error == WSAECONNABORTED || last_error == WSAETIMEDOUT ||
                       last_error == WSAECONNRESET || last_error == WSAECONNREFUSED || last_error == WSAENOTCONN || last_error == WSAENOBUFS);
...
void zmq::tcp_tune_loopback_fast_path(const fd_t socket_)
{
    ...
    const int rc =
        WSAIoctl(socket_, SIO_LOOPBACK_FAST_PATH, &sio_loopback_fastpath, sizeof sio_loopback_fastpath, NULL, 0, &number_of_bytes_returned, 0, 0);
...
zmq::fd_t zmq::tcp_open_socket(
    const char *address_, const zmq::options_t &options_, bool local_, bool fallback_to_ipv4_, zmq::tcp_address_t *out_tcp_addr_)
{
    ...
    //  Create the socket.（创建的是阻塞的 socket）
    fd_t s = open_socket(out_tcp_addr_->family(), SOCK_STREAM, IPPROTO_TCP);

    //  IPv6 address family not supported, try automatic downgrade to IPv4.
    if (s == retired_fd && fallback_to_ipv4_ && out_tcp_addr_->family() == AF_INET6 && errno == EAFNOSUPPORT && options_.ipv6)
    {
