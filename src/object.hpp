...
protected:
    ...
    void send_pipe_stats_publish(
        zmq::own_t *destination_, uint64_t outbound_queue_count_, uint64_t inbound_queue_count_, endpoint_uri_pair_t *endpoint_pair_);
...
private:
    ...
    /**
     * 创建 IO 线程或者回收线程或者 socket 线程的时候会初始化该 tid。
     * 每个需要收发命令的对象（有 mailbox_t 成员），都需要确定命令的发送方向，所以在实例化各个对象的时候，都会用自己的 tid 来初始化 object_t::_tid，以此
     * 来确定 send_command 的发送方向。
     *
     * 该 _tid 会决定将命令发送给 IO 线程、回收线程还是 socket
     */
    uint32_t _tid;
