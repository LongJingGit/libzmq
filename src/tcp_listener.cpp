...
int zmq::tcp_listener_t::create_socket(const char *addr_)
{
     ...
     *
     * SO_REUSEADDR 这个选项是用来通知内核，如果端口忙，且TCP连接状态处于TIME_WAIT，可以重用端口。如果端口忙，但是TCP端口处于其他状态，重用端口依然会得到一个错误信息（地址已经在使用中...）
     *
...
zmq::fd_t zmq::tcp_listener_t::accept()
{
    ...
    if (sock == retired_fd)
    {
        ...
        errno_assert(errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR || errno == ECONNABORTED || errno == EPROTO || errno == ENOBUFS ||
                     errno == ENOMEM || errno == EMFILE || errno == ENFILE || errno == EINVAL);
#else
        errno_assert(errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR || errno == ECONNABORTED || errno == EPROTO || errno == ENOBUFS ||
    ...
    {
        bool matched = false;
        for (options_t::tcp_accept_filters_t::size_type i = 0, size = options.tcp_accept_filters.size(); i != size; ++i)
        {
