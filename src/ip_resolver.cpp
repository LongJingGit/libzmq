...
#elif ((defined ZMQ_HAVE_LINUX || defined ZMQ_HAVE_FREEBSD || defined ZMQ_HAVE_OSX || defined ZMQ_HAVE_OPENBSD || defined ZMQ_HAVE_QNXNTO ||         \
           defined ZMQ_HAVE_NETBSD || defined ZMQ_HAVE_DRAGONFLY || defined ZMQ_HAVE_GNU) &&                                                         \
...
int zmq::ip_resolver_t::resolve_nic_name(ip_addr_t *ip_addr_, const char *nic_)
{
    ...
    for (const ifaddrs *ifp = ifa; ifp != NULL; ifp = ifp->ifa_next)
    {
        ...
        {
            memcpy(ip_addr_, ifp->ifa_addr, (family == AF_INET) ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6));
            found = true;
            break;
        }
    }
...
int zmq::ip_resolver_t::resolve_nic_name(ip_addr_t *ip_addr_, const char *nic_)
{
    ...
    do
    {
        ...
        rc = GetAdaptersAddresses(
            AF_UNSPEC, GAA_FLAG_SKIP_ANYCAST | GAA_FLAG_SKIP_MULTICAST | GAA_FLAG_SKIP_DNS_SERVER, NULL, addresses, &out_buf_len);
    ...
    if (rc == 0)
    {
        for (const IP_ADAPTER_ADDRESSES *current_addresses = addresses; current_addresses; current_addresses = current_addresses->Next)
        {
            char *if_name = NULL;
            ...
            if (((str_rc1 == 0) && (!strcmp(nic_, if_name))) || ((str_rc2 == 0) && (!strcmp(nic_, if_friendly_name))))
            {
                ...
                for (const IP_ADAPTER_UNICAST_ADDRESS *current_unicast_address = current_addresses->FirstUnicastAddress; current_unicast_address;
...
int zmq::ip_resolver_t::do_getaddrinfo(const char *node_, const char *service_, const struct addrinfo *hints_, struct addrinfo **res_)
{
