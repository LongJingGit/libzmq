...
zmq::session_base_t *zmq::session_base_t::create(
    class io_thread_t *io_thread_, bool active_, class socket_base_t *socket_, const options_t &options_, address_t *addr_)
{
    ...
    case ZMQ_CHANNEL:
        ...
        if (options_.can_send_hello_msg && options_.hello_msg.size() > 0)
            s = new (std::nothrow) hello_msg_session_t(io_thread_, active_, socket_, options_, addr_);
        else
...
void zmq::session_base_t::engine_error(bool handshaked_, zmq::i_engine::error_reason_t reason_)
{
    ...
    if (_pipe)
    {
        ...
        if (!_active && handshaked_ && options.can_recv_disconnect_msg && !options.disconnect_msg.empty())
        {
    ...
    zmq_assert(reason_ == i_engine::connection_error || reason_ == i_engine::timeout_error || reason_ == i_engine::protocol_error);
...
void zmq::session_base_t::reconnect()
{
    ...
    if (_pipe && options.immediate == 1
        ...
        && _addr->protocol != protocol_name::pgm && _addr->protocol != protocol_name::epgm
#endif
#ifdef ZMQ_HAVE_NORM
...
void zmq::session_base_t::start_connecting(bool wait_)
{
    ...
    if (_addr->protocol == protocol_name::tcp)
    {
        if (!options.socks_proxy_address.empty())
        {
            ...
            connecter = new (std::nothrow) socks_connecter_t(io_thread, this, options, _addr, proxy_address, wait_);
            alloc_assert(connecter);
            if (!options.socks_proxy_username.empty())
            {
                reinterpret_cast<socks_connecter_t *>(connecter)->set_auth_method_basic(options.socks_proxy_username, options.socks_proxy_password);
            }
        }
        else
        {
            connecter = new (std::nothrow) tcp_connecter_t(io_thread, this, options, _addr, wait_);
        }
    }
#if defined ZMQ_HAVE_IPC
    else if (_addr->protocol == protocol_name::ipc)
    {
        connecter = new (std::nothrow) ipc_connecter_t(io_thread, this, options, _addr, wait_);
    }
#endif
#if defined ZMQ_HAVE_TIPC
    else if (_addr->protocol == protocol_name::tipc)
    {
        connecter = new (std::nothrow) tipc_connecter_t(io_thread, this, options, _addr, wait_);
    }
#endif
#if defined ZMQ_HAVE_VMCI
    else if (_addr->protocol == protocol_name::vmci)
    {
        connecter = new (std::nothrow) vmci_connecter_t(io_thread, this, options, _addr, wait_);
    }
#endif
#if defined ZMQ_HAVE_WS
    else if (_addr->protocol == protocol_name::ws)
    {
        connecter = new (std::nothrow) ws_connecter_t(io_thread, this, options, _addr, wait_, false, std::string());
    }
#endif
#if defined ZMQ_HAVE_WSS
    else if (_addr->protocol == protocol_name::wss)
    {
        connecter = new (std::nothrow) ws_connecter_t(io_thread, this, options, _addr, wait_, true, _wss_hostname);
    }
#endif
    if (connecter != NULL)
    {
    ...
    {
        zmq_assert(options.type == ZMQ_PUB || options.type == ZMQ_XPUB || options.type == ZMQ_SUB || options.type == ZMQ_XSUB);
...
zmq::hello_msg_session_t::hello_msg_session_t(
    io_thread_t *io_thread_, bool connect_, socket_base_t *socket_, const options_t &options_, address_t *addr_)
...
int zmq::hello_msg_session_t::pull_msg(msg_t *msg_)
{
    if (_new_pipe)
    {
        ...
        const int rc = msg_->init_buffer(&options.hello_msg[0], options.hello_msg.size());
        errno_assert(rc == 0);
