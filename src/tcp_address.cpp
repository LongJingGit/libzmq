...
bool zmq::tcp_address_mask_t::match_address(const struct sockaddr *ss_, const socklen_t ss_len_) const
{
    ...
    if (_address_mask > 0)
    {
        ...
        if (ss_->sa_family == AF_INET6)
        {
            ...
            their_bytes = reinterpret_cast<const uint8_t *>(&((reinterpret_cast<const struct sockaddr_in6 *>(ss_))->sin6_addr));
        ...
        else
        {
            ...
            their_bytes = reinterpret_cast<const uint8_t *>(&((reinterpret_cast<const struct sockaddr_in *>(ss_))->sin_addr));
