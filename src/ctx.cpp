...
int zmq::ctx_t::terminate()
{
    ...
    if (!_starting)
    {
        ...
        if (_pid != getpid())
        {
            ...
            for (sockets_t::size_type i = 0, size = _sockets.size(); i != size; i++)
            {
...
bool zmq::ctx_t::start()
{
    ...
    for (int32_t i = static_cast<int32_t>(_slots.size()) - 1; i >= static_cast<int32_t>(ios) + term_and_reaper_threads_count; i--)
    {
...
zmq::socket_base_t *zmq::ctx_t::create_socket(int type_)
{
    ...
    //  Once zmq_ctx_term() or zmq_ctx_shutdown() was called, we can't create new sockets.
    if (_terminating)
    {
        errno = ETERM;
        return NULL;
...
int zmq::thread_ctx_t::get(int option_, void *optval_, const size_t *optvallen_)
{
    const bool is_int = (*optvallen_ == sizeof(int));
    int *value = static_cast<int *>(optval_);
...
int zmq::ctx_t::unregister_endpoint(const std::string &addr_, const socket_base_t *const socket_)
{
    scoped_lock_t locker(_endpoints_sync);
    ...
    zmq::socket_base_t *bind_socket_, const options_t &bind_options_, const pending_connection_t &pending_connection_, side side_)
{
    ...
    if (bind_options_.can_recv_disconnect_msg && !bind_options_.disconnect_msg.empty())
        pending_connection_.connect_pipe->set_disconnect_msg(bind_options_.disconnect_msg);
#endif
