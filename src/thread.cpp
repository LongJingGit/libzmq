...
{
    typedef EXCEPTION_DISPOSITION(NTAPI *HandlerFunctionType)(EXCEPTION_RECORD *, void *, CONTEXT *, void *);

    MY_EXCEPTION_REGISTRATION_RECORD *Next;
    HandlerFunctionType Handler;
};

static EXCEPTION_DISPOSITION NTAPI continue_execution(EXCEPTION_RECORD *rec, void *frame, CONTEXT *ctx, void *disp)
{
...
void zmq::thread_t::start(thread_fn *tfn_, void *arg_, const char *name_)
{
    ...
    _arg = arg_;
    _descriptor =
        taskSpawn(NULL, DEFAULT_PRIORITY, DEFAULT_OPTIONS, DEFAULT_STACK_SIZE, (FUNCPTR)thread_routine, (int)this, 0, 0, 0, 0, 0, 0, 0, 0, 0);
...
#    endif
    ...
    self->applySchedulingParameters();
    self->applyThreadName();
    self->_tfn(self->_arg); // 调用线程要执行的函数
    return NULL;
}
}
...
#        ifdef ZMQ_HAVE_PTHREAD_SET_AFFINITY
    if (!_thread_affinity_cpus.empty())
    {
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        for (std::set<int>::const_iterator it = _thread_affinity_cpus.begin(), end = _thread_affinity_cpus.end(); it != end; it++)
        {
