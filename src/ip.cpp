...
#    else
    int flag = 0;
#    endif
    const int rc = setsockopt(s_, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<char *>(&flag), sizeof(flag));
#    ifdef ZMQ_HAVE_WINDOWS
    wsa_assert(rc != SOCKET_ERROR);
#    else
    errno_assert(rc == 0);
...
int zmq::get_peer_ip_address(fd_t sockfd_, std::string &ip_addr_)
{
    ...
    const zmq_socklen_t addrlen = get_socket_address(sockfd_, socket_end_remote, &ss);
    ...
    const int rc = getnameinfo(reinterpret_cast<struct sockaddr *>(&ss), addrlen, host, sizeof host, NULL, 0, NI_NUMERICHOST);
    if (rc != 0)
        return 0;
...
void zmq::set_socket_priority(fd_t s_, int priority_)
{
    ...
    int rc = setsockopt(s_, SOL_SOCKET, SO_PRIORITY, reinterpret_cast<char *>(&priority_), sizeof(priority_));
    errno_assert(rc == 0);
#endif
}
...
{
    BOOL tcp_nodelay = 1;
    const int rc = setsockopt(socket_, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char *>(&tcp_nodelay), sizeof tcp_nodelay);
    wsa_assert(rc != SOCKET_ERROR);
...
#    ifdef __MINGW32__
        _snwprintf(mutex_name, MAX_PATH, L"Global\\zmq-signaler-port-%d", zmq::signaler_port);
#    else
        swprintf(mutex_name, MAX_PATH, L"Global\\zmq-signaler-port-%d", zmq::signaler_port);
#    endif
...
#    endif
    ...
    *r_ = INVALID_SOCKET;
    ...
    BOOL so_reuseaddr = 1;
    int rc = setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<char *>(&so_reuseaddr), sizeof so_reuseaddr);
    wsa_assert(rc != SOCKET_ERROR);
...
void zmq::assert_success_or_recoverable(zmq::fd_t s_, int rc_)
{
    ...
    if (err != 0)
    {
        wsa_assert(err == WSAECONNREFUSED || err == WSAECONNRESET || err == WSAECONNABORTED || err == WSAEINTR || err == WSAETIMEDOUT ||
                   err == WSAEHOSTUNREACH || err == WSAENETUNREACH || err == WSAENETDOWN || err == WSAENETRESET || err == WSAEACCES ||
    ...
    {
        errno = err;
        errno_assert(errno == ECONNREFUSED || errno == ECONNRESET || errno == ECONNABORTED || errno == EINTR || errno == ETIMEDOUT ||
                     errno == EHOSTUNREACH || errno == ENETUNREACH || errno == ENETDOWN || errno == ENETRESET || errno == EINVAL);
    }
#endif
}
