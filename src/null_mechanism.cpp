...
zmq::null_mechanism_t::null_mechanism_t(session_base_t *session_, const std::string &peer_address_, const options_t &options_)
...
int zmq::null_mechanism_t::process_error_command(const unsigned char *cmd_data_, size_t data_size_)
{
    ...
    {
        session->get_socket()->event_handshake_failed_protocol(session->get_endpoint(), ZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_ERROR);
