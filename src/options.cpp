...
static int do_getsockopt_curve_key(void *const optval_, const size_t *const optvallen_, const uint8_t (&curve_key_)[CURVE_KEYSIZE])
{
...
static int do_setsockopt_string_allow_empty_strict(
    const void *const optval_, const size_t optvallen_, std::string *const out_value_, const size_t max_len_)
{
...
int zmq::options_t::setsockopt(int option_, const void *optval_, size_t optvallen_)
{
    ...
    case ZMQ_IPV4ONLY: {
        bool value;
        const int rc = do_setsockopt_int_as_bool_strict(optval_, optvallen_, &value);
        if (rc == 0)
            ipv6 = !value;
        return rc;
    }
...
#    ifdef ZMQ_HAVE_WSS
    ...
    case ZMQ_WSS_TRUST_SYSTEM:
        return do_setsockopt_int_as_bool_strict(optval_, optvallen_, &wss_trust_system);
#    endif
    ...
    case ZMQ_DISCONNECT_MSG:
        ...
        {
            unsigned char *bytes = (unsigned char *)optval_;
            disconnect_msg = std::vector<unsigned char>(bytes, bytes + optvallen_);
        }
        else
        {
